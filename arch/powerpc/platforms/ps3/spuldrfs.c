	...
	size_t size, loff_t *pos)
{
	...
	len = sprintf(buf, "buf1 %lx\nbuf2 %lx\nbuf3 %lx",
		(unsigned long) spuldrfs_spe_buf1, (unsigned long) spuldrfs_spe_buf2, (unsigned long) spuldrfs_spe_buf3);
...
static irqreturn_t spuldrfs_spe_interrupt(int irq, void *data)
{
	...
	if (irq == spuldrfs_spe_virq[0]) {
		...
		if (status & CLASS1_SEGMENT_FAULT_INTR) {
			...
			vsid = (get_kernel_vsid(ea, MMU_SEGSIZE_256M) << SLB_VSID_SHIFT) | SLB_VSID_KERNEL | MMU_PAGE_4K;

			printk(KERN_INFO"spuldrfs: data segment fault at %llx\n", ea);
		...
		if (status & CLASS1_STORAGE_FAULT_INTR) {
			...
			printk(KERN_INFO"spuldrfs: data storage fault at %llx\n", ea);

			if (dsisr & MFC_DSISR_PTE_NOT_FOUND) {
				err = hash_page(ea, _PAGE_PRESENT, 0x300);
				if (err) {
...
{
	u64 ppe_id, vas_id, junk;
	int err;

	err = lv1_get_logical_ppe_id(&ppe_id);
	if (err)
		return (-ENXIO);

	err = lv1_get_virtual_address_space_id_of_ppe(ppe_id, &vas_id);
	if (err)
		return (-ENXIO);
